...
def plot_bboxes(image, boxes, fname, labels={}, colors=[], score=True, conf=None, ):
    # Define COCO Labels
    if not labels:
        labels = {0: u'shot', 1: u'person', 2: u'bicycle', 3: u'car', 4: u'motorcycle', 5: u'airplane',
    ...
    #cv2.imwrite(fr'C:\Users\fziet\OneDrive\Pulpit\praca\model\yolov8m-flipped\{fname}_predicted.jpg', image)


opoznienie = 0
cap = cv2.VideoCapture(f'showcase.mp4')  # załaduj film
model = YOLO(r"C:\Users\fziet\OneDrive\Pulpit\szkola\inzynierka\shot_detection\runs\detect\train3\weights\best.pt")
fps = cap.get(cv2.CAP_PROP_FPS)
codec = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
_, frame = cap.read()
out = cv2.VideoWriter(r'C:\Users\fziet\OneDrive\Pulpit\szkola\inzynierka\shot_detection\demo\demo-80.mp4', codec, fps,
                      (frame.shape[1], frame.shape[0]))
i = 0
while cap.isOpened():  # iteruje po klatakch filmu
    ret, frame = cap.read()
    if ret:
        results = model(frame)  # dokonaj wykrywania obiektow
        i += 1
        plot_bboxes(frame, results[0].boxes.boxes, opoznienie, labels={0: 'shot'}, score=True, conf=0.79)
        out.write(frame)
    else:
        break
cap.release()
